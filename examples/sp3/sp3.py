...
def sim(N, ratio):
  ...
  solver.minimize_BB((L/N) ** 3)
  #solver.minimize()
  print "Etot", solver.state.E_minimize_BB
  writeVTK("vtr/state-vortex-%s-%s.vtr" % (N, ratio), solver.state.M)
  ...
  solver.minimize_BB((L/N) ** 3)
  print "Etot", solver.state.E_minimize_BB
  writeVTK("vtr/state-flower-%s-%s.vtr" % (N, ratio), solver.state.M)
